...
class BaseConfigTracking(BaseConfigCircle):
    """EEG Spatial Attention Tracking config.
    ...
    def __init__(self):
        ...
        self.task = 'attention_tracking'
        self.data = 'udf_viz_openimages'
        ...
        # ── Projection head (decoupled from cls_head for tracking) ──────
        self.feat_dim = 16          # backbone embedding dimension
        self.proj_dim = 512         # Must match CLIP Image Embeddings (e.g. 512)
        self.bbox_dim = 4           # Spatial coordinates regression size
        ...
        self.comment = f'{datetime.now().strftime("%Y%m%d_%H%M%S")}-tracking-{self.model}-T{self.t0}'
